...
@section('navbar-title', 'Status Selesai')
...
<div class="table-card">
    ...
    <div class="table-header">
        ...
        <div style="display:flex; gap:12px; align-items:center;">
            ...
            {{-- FILTER TANGGAL --}}
            <x-filter-tanggal 
                :action="route('siswa.status.selesai')" 
                :preserve="request()->only('sort')" 
            />

            {{-- SORT --}}
            <form method="GET" action="{{ route('siswa.status.selesai') }}" class="sort-wrapper">
...
{{-- ================= LIVE SEARCH ================= --}}
<script>
const searchInput = document.getElementById('searchSelesai');
const tableBody   = document.getElementById('selesaiTable');
...
searchInput.addEventListener('keyup', function () {
    ...
    timer = setTimeout(() => {
        ...
        fetch(`/siswa/status/selesai/search?search=${encodeURIComponent(keyword)}`)
            .then(res => res.json())
            .then(data => {
